...
DROP TABLE `MOVIE_TICKET` CASCADE CONSTRAINTS;
...
CREATE TABLE `MOVIE_TICKET` (
    `SCREEN_CODE` VARCHAR2(50) NOT NULL,
    `SEAT_CODE` VARCHAR2(50) NOT NULL,
    `MT_STATUS` VARCHAR2(20) NOT NULL,
    PRIMARY KEY (`SCREEN_CODE`, `SEAT_CODE`)
);
...
ALTER TABLE `TICKETING` ADD FOREIGN KEY (`MT_CODE`) REFERENCES `SCREEN`(`SCREEN_CODE`);
...
ALTER TABLE `MOVIE_TICKET` ADD FOREIGN KEY (`SCREEN_CODE`) REFERENCES `SCREEN`(`SCREEN_CODE`);
ALTER TABLE `MOVIE_TICKET` ADD FOREIGN KEY (`SEAT_CODE`) REFERENCES `SEAT`(`SEAT_CODE`);