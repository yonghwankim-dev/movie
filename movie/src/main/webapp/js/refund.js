	...
	$.ajax({
		...
		success : function(res){
			...
			$.each(res.datas, function(i,v){
				...
				code += '      <a idx="' + v.ref_sc_num + '" class="list" data-toggle="collapse" data-parent="#accordion" href="#collapse' + v.ref_sc_num + '">';
				code += 		v.ref_sc_tit + '</a>';      
				code += '    </h4>';
				code += '  </div>';
				code += '  <div id="collapse' + v.ref_sc_num + '" class="panel-collapse collapse">';
				code += '    <div class="panel-body pbody">';
				code += '		<div class="p1">작성자 : ' + v.ref_sc_wri + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				code += '									회원 코드 : ' + v.mem_cd + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				code += '									비회원 코드 : ' + v.nmem_cd + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				code += '		</div>';
				code += '		<div class="p2">';
				code += '  			<input idx="' + v.ref_sc_num + '" type="button" name="modify" value="수정" class="action btn btn-warning btn-sm">';
				code += '			<input idx="' + v.ref_sc_num + '" type="button" name="delete" value="삭제" class="action btn btn-danger btn-sm">';
				code += '		</div>';
				code += '		<div class="p3">' + v.ref_sc_con + '</div>';
				code += '    </div>';
				code += '  </div>';
				code += '</div>';
			});
		    code += '</div>';
		    $('#refundList').html(code);
...
$(function(){
	...
	$('#refundList').on('click','.action', function(){
